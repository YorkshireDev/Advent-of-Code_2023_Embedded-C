...
void runOnce()
{
  char* num_arr[] = {"one", "two", "three", "four", "five", "six", "seven", "eight", "nine"};
  char num_build_str[58];
  ...
  for (unsigned short i = 0; i < sizeof(input_list) / sizeof(input_list[0]); i++)
  {
    ...
    num_build_str[0] = '\0';
    unsigned short num_build_str_index = 0;
    ...
    for (size_t char_index = 0; char_index < strlen(str); char_index++)
    {
      ...
      {
        num_build_str[num_build_str_index++] = str[char_index];
        num_build_str[num_build_str_index] = '\0';
        
        for (unsigned short num_arr_index = 0; num_arr_index < sizeof(num_arr) / sizeof(num_arr[0]); num_arr_index++)
        { 
          if (strstr(num_build_str, num_arr[num_arr_index]) != NULL)
          {
            last_num = '0' + (num_arr_index + 1);
            if (first_num == 'A')
            {
              first_num = last_num;
            }

            num_build_str_index = 0;
            num_build_str[num_build_str_index] = str[char_index];
            num_build_str[++num_build_str_index] = '\0';
            break;
          }
